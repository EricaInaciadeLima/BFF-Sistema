import { ListarPedidoGateway } from './listar-pedido.gateway';
...
describe('ListarPedidoGateway', () => {
  let gateway: ListarPedidoGateway;
  let mockFetch: jest.Mock;

  beforeEach(() => {
    gateway = new ListarPedidoGateway();
    mockFetch = jest.fn();
  ...
  describe('construção de URL — parâmetros obrigatórios', () => {
    test.each([
      {
        descricao: 'page, size e sort sempre incluídos',
  ...
  describe('sucesso — retorno dos dados', () => {
    it('deve retornar os dados parseados da resposta', async () => {
      const respostaApi = {
        content: [
          { id: 2, cliente: 'Bruno', status: 'ENTREGUE', momento: '2026-06-10', valorTotal: 120 },
        ],
        number: 0,
        size: 10,
        totalElements: 1,
